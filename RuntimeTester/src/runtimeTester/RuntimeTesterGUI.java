...
{
	public static final String RUNTIME_TESTER_FXML_FILENAME = "RuntimeTester.fxml";
	...
	public RuntimeTesterGUI() throws FileNotFoundException
	{
		...
		preRunCodeProcessor = new UserInputCodeProcessor(Utils.fileToString(new File("ClassTemplates/PreRunCodeTemplate.template")), "PreRunCode", USER_INPUT_PROCESSOR_OUTPUT_DIR);
		incrementationCodeProcessor = new UserInputCodeProcessor(Utils.fileToString(new File("ClassTemplates/IncrementationCodeTemplate.template")), "IncrementationCode", USER_INPUT_PROCESSOR_OUTPUT_DIR);
		parameterSetterProcessor = new UserInputCodeProcessor(Utils.fileToString(new File("ClassTemplates/ParameterSetterCode.template")), "ParameterSetter", USER_INPUT_PROCESSOR_OUTPUT_DIR);
		
		runtimeTester = new RuntimeTester(20, 15, incrementationCode, preRunCode);
	}
	...
	public void start(Stage primaryStage)
	{
		...
		try
		{
			FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource(RUNTIME_TESTER_FXML_FILENAME));
	...
	public void run(int iterations, int precision)
	{
		try
		{
			...
			{
				params[i] = runtimeTesterParameter.get(i).getParameter(environmentVariableStorage);
			}
			
			runtimeTester.run(instance, environmentVariableStorage, environmentVariableStorage, loadedMethod, params);
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	public void updatePreRunCode(String userInput)
	{
		try
		{
			...
			{
				preRunCode = (IPreRunPhase) cls.getConstructors()[0].newInstance();
			}
		}
	...
	public void updateIncrementationCode(String userInput)
	{
		try
		{
			...
			if(cls.getConstructors().length == 1)
			{
				incrementationCode = (IIncrementationPhase) cls.getConstructors()[0].newInstance();
			}
		}
		catch (FileNotFoundException e)
		{
	...
	public void updateRuntimeTesterParameters(ArrayList<String> parameters)
	{
		...
		for(String parameter: parameters)
		{
			try
			{
				...
				if(cls.getConstructors().length == 1)
				{
					IParameterSetter parameterSetter = (IParameterSetter) cls.getConstructors()[0].newInstance();
